...
namespace thermalprinting
{
    ...
    {    
        ...
        {
            const int COST_TAM = 40;
            const int COST_NCF = 19;
            ...
            if (company["data"].HasValues)
            {
                ...
                {
                    report.AppendLine(PadBoth(oData["address"].Value<string>(), COST_TAM));
                }
                if (oData["rnc"] != null)
                {
            ...
            if (sales["data"].HasValues)
            {
                if (sales["data"]["items"].HasValues)
                {
                    ...
                    report.Append(string.Format("{0:N2}", "totalitbis").PadLeft(second_col_pad));
                    ...
                    //report.AppendLine(string.Format("{0:N2}", dtTable.Rows[0]["Pago"]).PadLeft(third_col_pad));

                    strLineaFinal = "CAMBIO".PadRight(name_length, ' ');
                    report.Append(strLineaFinal.PadRight(first_col_pad));
                    report.Append(" ".PadLeft(second_col_pad));
                    //report.AppendLine(string.Format("{0:N2}", dtTable.Rows[0]["Cambio"]).PadLeft(third_col_pad));
                    report.AppendLine("---------------------------------------");
